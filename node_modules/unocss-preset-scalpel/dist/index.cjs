...
function setConfig$1(config) {
  presetConfig$1 = config;
}
function getDirectionOrder(order, direction) {
  if (!direction) {
    return order;
  }
  switch (direction) {
...
function generatorLayer(order) {
  return { layer: String(order).padStart(5, "0") };
}
function v2any(num) {
  const newNum = Number(num);
  const { rootValue, unitPrecision, minPixelValue } = presetConfig$1.vToAny;
  if (newNum < minPixelValue) {
    return num;
  }
  return new Decimal(newNum).div(rootValue).toFixed(unitPrecision);
}
function getUnitAndNum(unit, num) {
  const defaultUnit = presetConfig$1.unit;
  let newUnit = unit;
  let newNum = num;
  if (Number(num) === 0) {
    newUnit = "";
  ...
  if (newUnit === "v") {
    newUnit = presetConfig$1.vToAny.unit;
    newNum = v2any(num);
  }
  return `${newNum}${newUnit}`;
}
function ConvertToCssObject(cssList) {
  return Object.fromEntries(cssList.map((css) => {
    return css.split(":");
  }));
}
...
function textToRgbText(str, opacity = 1) {
  const hex = /^#?([a-fA-F0-9]{8}|[a-fA-F0-9]{6}|[a-fA-F0-9]{3})$/.test(str) ? str.replace(/^#/, "") : presetConfig$1.colors[str].replace(/^#/, "");
  if (hex === "transparent") {
    return "transparent";
  }
  if (hex.length === 6) {
  ...
  if (hex.length === 8) {
    const reg = /[a-fA-F0-9]{2}/g;
    const [r, g, b, a] = hex.match(reg);
    const opacityA = Number(Number(Math.round(parseInt(a, 16)) / 255).toFixed(2));
    return `rgba(${[r, g, b].map(radix16).join(",")},${opacityA})`;
  }
  return "";
}

function getVariantImportant(config) {
  return (matcher) => {
...
function getVariantMediaQuery(config) {
  return (matcher) => {
    const [, mediaQuery = ""] = matcher.match(new RegExp(`(${Object.keys(config.mediaQueries).join("|")})@`)) ?? [];
    if (!mediaQuery) {
      return { matcher };
    }
    return {
...
const GAP_ENUM = ["unset", "initial", "inherit", "normal"];
const VERTICAL_ALIGN_ENUM = [
  "baseline",
  "sub",
  "super",
  "text-top",
...
const JUSTIFY_CONTENT_ENUM_STR = toRegexStr(JUSTIFY_CONTENT_ENUM);
const ALIGN_ITEMS_ENUM_STR = toRegexStr(ALIGN_ITEMS_ENUM);
const CURSOR_ENUM_STR = toRegexStr(CURSOR_ENUM);
const UNIT_ENUM_STR = toRegexStr(UNIT_ENUM);
const VERTICAL_ALIGN_STR = toRegexStr(VERTICAL_ALIGN_ENUM);
const GAP_ENUM_STR = toRegexStr(GAP_ENUM);
const WHITE_SPACE_ENUM_STR = toRegexStr(WHITE_SPACE_ENUM);
const NONNEGATIVE_NUMBER_REGEX_STR = "(0|([1-9]\\d*))(\\.\\d*[1-9])?";
...
function getCss(direction, value) {
...
function handleBorder({ groups }) {
  const { direction, num, unit } = groups;
  return ConvertToCssObject(getCss(direction, getUnitAndNum(unit, num)));
}
const border = () => [
  [
    new RegExp(`^(border|border-width|border-w)-(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?$`),
    handleBorder,
    {
  ...
  [
    new RegExp(`^(border|border-width|border-w)-(?<direction>[xy])-(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?$`),
    handleBorder,
    {
  ...
  [
    new RegExp(`^(border|border-width|border-w)-(?<direction>[trbl])-(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?$`),
    handleBorder,
    {
...
const borderRadius = () => [
  [
    new RegExp(`^(border-radius|br)-(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?$`),
    ({ groups }) => {
  ...
  [
    new RegExp(`^(border-radius|br)-((?<direction>[trbl])-)(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?$`),
    ({ groups }) => {
  ...
  [
    new RegExp(`^(border-radius|br)-((?<direction>(tl|lt|tr|rt|bl|lb|br|rb))-)(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?$`),
    ({ groups }) => {
      const { num, unit, direction } = groups;
      switch (direction) {
        case "tl":
...
const borderStyle = () => [
  [
    ...
    {
      ...generatorLayer(550),
      autocomplete: "border(-style|)-(none|hidden|dotted|dashed|solid|double|groove|ridge|inset|outset|inherit)"
    }
  ]
];
...
function handleColor({ groups }) {
  const { type, color: color2, opacity } = groups;
  const newOpacity = opacity === void 0 ? 1 : Number((Number(opacity) * 0.01).toFixed(2));
  const newColor = textToRgbText(color2, newOpacity);
...
const color = (config) => {
  ...
  return [
    [
      new RegExp(`^(?<type>color|c|text)-(?<color>(#?([a-fA-F0-9]{8}$|[a-fA-F0-9]{6}|[a-fA-F0-9]{3}))|${colorKeysStr})$`),
      handleColor,
      {
        ...generatorLayer(900),
    ...
    [
      new RegExp(`^(?<type>color|c|text)-(?<color>(#?([a-fA-F0-9]{8}$|[a-fA-F0-9]{6}|[a-fA-F0-9]{3}))|${colorKeysStr})-(?<opacity>1|([1-9]\\d?))$`),
      handleColor,
      {
        ...generatorLayer(910),
    ...
    [
      new RegExp(`^(?<type>bg|background)-(?<color>(#?([a-fA-F0-9]{8}$|[a-fA-F0-9]{6}|[a-fA-F0-9]{3}))|${colorKeysStr})$`),
      handleColor,
      {
        ...generatorLayer(920),
    ...
    [
      new RegExp(`^(?<type>bg|background)-(?<color>(#?([a-fA-F0-9]{8}$|[a-fA-F0-9]{6}|[a-fA-F0-9]{3}))|${colorKeysStr})-(?<opacity>1|([1-9]\\d?))$`),
      handleColor,
      {
        ...generatorLayer(930),
    ...
    [
      new RegExp(`^(?<type>border-c|border-color|border)-(?<color>(#?([a-fA-F0-9]{8}$|[a-fA-F0-9]{6}|[a-fA-F0-9]{3}))|${colorKeysStr})$`),
      handleColor,
      {
        ...generatorLayer(940),
    ...
    [
      new RegExp(`^(?<type>border-c|border-color|border)-(?<color>(#?([a-fA-F0-9]{8}$|[a-fA-F0-9]{6}|[a-fA-F0-9]{3}))|${colorKeysStr})-(?<opacity>1|([1-9]\\d?))$`),
      handleColor,
      {
        ...generatorLayer(950),
        autocomplete: "(border-c|border-color|border)-(red|blue|#F00|F00)-<num>"
      }
    ]
  ];
};
...
const columnGap = () => [
  [
    new RegExp(`^c(olumn)?-gap-(((?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?)|(?<value>${GAP_ENUM_STR}))$`),
    ({ groups }) => {
      const {
        num = Infinity,
        unit,
...
const flexBasis = () => [
  [
    new RegExp(`^flex-basis-(?<value>((?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?)|initial|inherit|auto)$`),
    ({ groups }) => {
...
const flexDirection = () => [
  [
    ...
    {
      ...generatorLayer(230),
      autocomplete: "(flex-direction|flex)-(row|row-reverse|column|column-reverse)"
    }
  ]
];
...
const flexJustAli = () => [
  [
    new RegExp(`^flex-(?<justify>${JUSTIFY_CONTENT_ENUM_STR})-(?<align>${ALIGN_ITEMS_ENUM_STR})$`),
    ({ groups }) => {
...
const fontSize = () => [
  [
    new RegExp(`^(font-size|fs)-(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?$`),
    ({ groups }) => {
      const { num, unit } = groups;
...
const gap = () => [
  [
    new RegExp(`^gap-(((?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?)|(?<value>${GAP_ENUM_STR}))$`),
    ({ groups }) => {
      const { num, unit, value } = groups;
      const newValue = value || getUnitAndNum(unit, num);
      return ConvertToCssObject([
        `column-gap: ${newValue}`,
        `row-gap: ${newValue}`
...
const height = () => [
  [
    new RegExp(`^(h|height)-(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?$`),
    ({ groups }) => {
      const { num, unit } = groups;
...
const letterSpacing = () => [
  [
    new RegExp(`^letter-spacing-(?<isMinus>m-)?(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?$`),
    ({ groups }) => {
      const { isMinus, num, unit } = groups;
...
const lineHeight = () => [
  [
    new RegExp(`^(lh|line-height)-(?<value>((?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?)|normal|unset|inherit|initial)$`),
    ({ groups }) => {
      const { value, num, unit } = groups;
      const newValue = num ? `${getUnitAndNum(unit, num)}` : value;
...
const marginAndPadding = () => [
  [
    new RegExp(`^(?<type>m|margin)-((?<auto>auto)|(?<isMinus>m-)?(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?)$`),
    ({ groups }) => {
      const { auto, isMinus, num, unit } = groups;
      let value = "";
      if (isMinus) {
  ...
  [
    new RegExp(`^(?<type>m|margin)-((?<direction>[xy])-)((?<auto>auto)|(?<isMinus>m-)?(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?)$`),
    ({ groups }) => {
      const { auto, isMinus, num, unit, direction } = groups;
      let value = "";
      if (isMinus) {
        value = `${getUnitAndNum(unit, 0 - Number(num))}`;
      } else {
      ...
      return ConvertToCssObject(DIRECTION_MAP.get(direction).reduce((t, c) => [...t, `margin-${c}: ${value}`], []));
    },
    {
  ...
  [
    new RegExp(`^(?<type>m|margin)-((?<direction>[tbrl])-)((?<auto>auto)|(?<isMinus>m-)?(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?)$`),
    ({ groups }) => {
      const { auto, isMinus, num, unit, direction } = groups;
      let value = "";
      if (isMinus) {
  ...
  [
    new RegExp(`^(?<type>p|padding)-((?<auto>auto)|(?<isMinus>m-)?(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?)$`),
    ({ groups }) => {
      const { auto, isMinus, num, unit } = groups;
      let value = "";
      if (isMinus) {
  ...
  [
    new RegExp(`^(?<type>p|padding)-((?<direction>[xy])-)((?<auto>auto)|(?<isMinus>m-)?(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?)$`),
    ({ groups }) => {
      const { auto, isMinus, num, unit, direction } = groups;
      let value = "";
      if (isMinus) {
      ...
      return ConvertToCssObject(DIRECTION_MAP.get(direction).reduce((t, c) => [...t, `padding-${c}: ${value}`], []));
    },
    {
  ...
  [
    new RegExp(`^(?<type>p|padding)-((?<direction>[tbrl])-)((?<auto>auto)|(?<isMinus>m-)?(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?)$`),
    ({ groups }) => {
      const { auto, isMinus, num, unit, direction } = groups;
      let value = "";
      if (isMinus) {
...
const minMaxHeightWidth = () => [
  [
    new RegExp(`^(max-h|max-height)-(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?$`),
    ({ groups }) => {
      const { num, unit } = groups;
  ...
  [
    new RegExp(`^(min-h|min-height)-(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?$`),
    ({ groups }) => {
      const { num, unit } = groups;
  ...
  [
    new RegExp(`^(max-w|max-width)-(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?$`),
    ({ groups }) => {
      const { num, unit } = groups;
  ...
  [
    new RegExp(`^(min-w|min-width)-(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?$`),
    ({ groups }) => {
      const { num, unit } = groups;
...
const orientation = () => [
  [
    new RegExp(`^(?<direction>[trbl]|top|right|bottom|left)-(?<isMinus>m-)?(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?$`),
    ({ groups }) => {
      const { direction, isMinus, num, unit } = groups;
      const newNum = isMinus ? `-${num}` : num;
      const newDirection = direction.length === 1 ? DIRECTION_MAP.get(direction)[0] : direction;
      return ConvertToCssObject([
...
const position = () => [
  [
    ...
    {
      ...generatorLayer(300),
      autocomplete: "position-(static|relative|sticky|unset|absolute|fixed|inherit|initial)"
    }
  ]
];
...
const rowGap = () => [
  [
    new RegExp(`^r(ow)?-gap-(((?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?)|(?<value>${GAP_ENUM_STR}))$`),
    ({ groups }) => {
      const { num, unit, value } = groups;
      const newValue = value || getUnitAndNum(unit, num);
...
const square = () => [
  [
    new RegExp(`^square-(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?$`),
    ({ groups }) => {
      const { num, unit } = groups;
      return ConvertToCssObject([
...
const textAlign = () => [
  [
    ...
    {
      ...generatorLayer(320),
      autocomplete: "(text-align|text)-(start|end|left|right|center|justify|match-parent)"
    }
  ]
];
...
const textAlignLast = () => [
  [
    ...
    {
      ...generatorLayer(440),
      autocomplete: "(text-align-last|text-last)-(auto|left|right|center|justify|start|end|initial|inherit)"
    }
  ]
];
...
const textDecoration = () => [
  [
    ...
    {
      ...generatorLayer(420),
      autocomplete: "(text-decoration|text)-(none|underline|overline|line-through|blink|inherit)"
    }
  ]
];
...
const verticalAlign = () => [
  [
    new RegExp(`^vertical-align-(?<value>((?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?)|${VERTICAL_ALIGN_STR})$`),
    ({ groups }) => {
      const { value, num, unit } = groups;
      const newValue = num ? `${getUnitAndNum(unit, num)}` : value;
...
const width = () => [
  [
    new RegExp(`^(w|width)-(?<num>${NONNEGATIVE_NUMBER_REGEX_STR})(?<unit>${UNIT_ENUM_STR})?$`),
    ({ groups }) => {
      const { num, unit } = groups;
...
const wordBreak = () => [
  [
    ...
    {
      ...generatorLayer(350),
      autocomplete: "word-break-(normal|break-all|keep-all|break-word|inherit|initial|unset)"
    }
  ]
];
...
function setConfig(config) {
  ...
  setConfig$1(presetConfig);
}
function presetScalpel(options) {
  setConfig(options ?? {});
  return {
    ...
    rules: Object.entries(rules).reduce((t, c) => {
      const [name, value] = c;
      return presetConfig.ignoreRules.includes(name) ? t : [...t, value(presetConfig)];
    }, []).flat(1)
  };
}

exports.presetScalpel = presetScalpel;
