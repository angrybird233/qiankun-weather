...
const utilities = require('./unocss-preset-weapp.42d31136.cjs');
const utils = require('unplugin-transform-class/utils');
const ruleUtils = require('@unocss/rule-utils');
const transform = require('./unocss-preset-weapp.9954e238.cjs');
...
const bgColors = [
  [/^bg-(.+)$/, (params, body) => {
    ...
    if (bgGradientRE.test(d) || bgImageRE.test(d)) {
      const s = utilities.handler.bracket(d);
      if (s) {
        const url = s.startsWith("http") ? `url(${s})` : utilities.handler.cssvar(s);
